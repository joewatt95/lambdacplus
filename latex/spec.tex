...
\begin{enumerate}
  ...
  \item Elimination \\
  The typing rules for the eliminatros \verb|fst| and \verb|snd| are
  \[
  \begin{prooftree}
    \hypo{p : A \times B}
    \infer1{\fst p : A}
...
When we come to dependent types later, we will see that these rules will become
significantly more complex.

The Curry-Howard correspondence can be summarized by the following table.
...
In order to implement our type system, we promote types to the level of
expressions in our language, so we no longer have separate syntactic categories 
for them.

This means that \textit{everything} in our language is an expression, even
...
\info{
Note that \tt{(let x := E in E')} expressions are not considered to be in head 
...
\info{
Note that the lack of a normalization rule for \texttt{Kind} is deliberate.
...
\begin{definition} [Well formed type and type constructor]
  ...
  $\Kind$ is just here mainly to be the type of $\Type$.
  }
...
\info{
  ...
  This means that we will be normalizing expressions before storing them in the
  context.
}
...
\item \textbf{Check} \\
  ...
  \info{
    Note that to compare 2 expressions that are in head normal form for equality,
    we only need to compare them structurally, modulo alpha equivalence.
    This is the reason why we prefer to eagerly normalize all types as soon
    as we verify that they are well formed. It makes the check for equality
    simple.
  }
...
% provided $x$ does not occur free in $\Gamma$ and $\psi$.
  ...
  coproduct, out of types that live in the universe \texttt{verb}, not 
  \verb|Kind|.
  ...
  type constructors found in \texttt{verb}.
  }
...
\item \textbf{Initial configuration:}
  ...
  \Type is used as a dummy initial output value here.
...
\item \textbf{Axiom} \\
  ...
  Note that \texttt{und} here indicates that $x$ has no binding, like an unknown
  constant.

  The output of an $\axiom$ expression is the normalized form of the type of 
...
Our interpreter is written in Ocaml and the structure is as follows.

\begin{itemize}
\tightlist
\item
  \texttt{bin} currently contains only \texttt{main.ml} which contains
  the main entry point to interact with our language.
\item
  \texttt{lib} is our library which contains all the code making our
  language work.

  \begin{itemize}
  \tightlist
  \item
    \texttt{parsing} contains all the functions we use for parsing.

    \begin{itemize}
    \tightlist
    \item
      \texttt{lexer.ml} contains our lexer.
    \item
      \texttt{grammar.mly} contains our grammar, written using Menhir.
    \item
      \texttt{parser.ml} contains some functions and boilerplate code
      tying the lexer and grammar together. This provides an interface
      which we use to parse our language.
    \end{itemize}
  \item
    \texttt{kernel} contains all the important stuff for evaluating
    expressions and statements. These include normalization, type
    checking and context management using de bruijn indices.

    \begin{itemize}
    \tightlist
    \item
      \texttt{context.ml} is the module implementing the
      context/environment.
    \item
      \texttt{normalization.ml} currently contains the
      \texttt{normalize}, \texttt{subst} and \texttt{beta\_reduce}
      functions. The latter 2 implement the substitution operation for
      de bruijn ASTs.
    \item
      \texttt{typing.ml} implements our bidirectional typechecking
      algorithm. This indluces the 2 key functions \texttt{check} and
      \texttt{infer}, where \texttt{infer} involves synthesizing the
      type, while \texttt{check} involves verifying if an expression has
      a given type.
    \item
      \texttt{eval\_statements.ml} contains functions for evaluating
      statements.
    \end{itemize}
  \item
    \texttt{common} contains code that is shared between both
    \texttt{kernel} and \texttt{parsing}.

    \begin{itemize}
    \tightlist
    \item
      \texttt{location.ml} contains a \texttt{located} datatype and
      other stuff which we use to decorate our AST in \texttt{ast.ml}
      with source locations. This info will be used for error reporting.
    \item
      \texttt{ast.ml} contains our AST. It is parameterized over a type
      variable, \texttt{\textquotesingle{}a}, where
      \texttt{\textquotesingle{}a} can either be \texttt{string} or
      \texttt{int}. This type variable denotes the type of variable
      identifiers. The AST which our parser parses the concrete syntax
      into uses \texttt{string} to identify variables, while our
      internal one, used for typechecking and normalization, uses
      \texttt{int} denoting de bruijn indices to identify variables.
    \end{itemize}
  \item
    \texttt{ast\_conv.ml} contains facilities for converting the
    parser's AST to the internal AST and vice versa.
  \item
    \texttt{error\_reporting.ml} contains functions for handling errors
    that occur while running programs in our language. These include
    pretty printing of errors.
  \item
    \texttt{pretty\_printing.ml} contains utilities for unparsing
    expressions and pretty printing them.
  \end{itemize}
\end{itemize}
