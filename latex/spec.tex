...
Our language is a playground for the Curry-Howard which implements an impredicative type
theory based on the Calculus of Constructions (CoC).

Following CoC, we stay true to the spirit of the 
\textit{Curry-Howard correspondence} and identify propositions as data types.
...
As for the existential quantifier, the BHK interpretation says that it's a 
...
The Sigma type generalizes the simple procuct type for pairs in the simply
typed lambda calculus to Sigma type, $\Sigma_{x : A} P(x)$.
The difference here is that the type of the second component to depend on the
value of the first.
...
The ordinary existential quantifier that we know from logic can be modeled by
formulating the elimination rule in a slightly different manner. 
...
The answer lies in a feature that modern functional languages like Haskell and
Ocaml offer -- existential types. 
...
with an unpacking, ie elimination, rule that looks like:
\[
  \begin{prooftree}
    \hypo{\Gamma \vdash t_1 : \{\exists X, \, T_{12} \}} 
    \hypo{\Gamma, \, X, \, x : T_{12} \, \vdash t_2 : T_2}
    \infer2{\Gamma \vdash \text{let } \{X, \, x = t_1\} \text{ in } t_2 : T_2}
  \end{prooftree} 
\]
Note that $X$ may occur free in $T_{12}$ here since $T_{12}$ is existentially
quantified over the type variable $X$.
...
which eerily resembles the usual rule of existential elimination from logic:
\[
\begin{prooftree}
...
In other words, we may derive the desired conclusion, $\psi$, 
...
quantifier is a dependent version of the existential type. 
...
Now, we may reformulate the rule of existential elimination
using the Deduction and Generalization theorems or simply from the
rules of intuitionistic natural deduction, as
\[
\begin{prooftree}
  \hypo{\Gamma \vdash \exists x, \, \varphi(x)}
  \hypo{\Gamma \vdash \forall x \, (\varphi(x) \rightarrow \psi)}
  \infer2{\Gamma \vdash \psi}
\end{prooftree}
\]
provided $x$ does not occur free in $\psi$.

Next if we follow the BHK interpretation and translate this into type theory,
we end up with a variation of a dependent pair with a weaker elimination
rule.
To distinguish these from the previous, stronger, Sigma type, we call these
\textit{existentials} and we write $\exists x : A, \, B(x)$ instead of 
$\Sigma_{x : A} B(x)$ and we denote elements of this type by $(|x, \, y|)$ to
differentiate them from the stronger variation.

If we then translate the universal quantifier and implication into the Pi type
and make explicit the proofs of these propositions/types, we arrive at the type
theoretic rule of existential elimination:
\[
  \begin{prooftree}
    \hypo{\Gamma \vdash E_1 : \exists x : A, \, B(x) }
    \hypo{\Gamma \vdash E_2 : \Pi_{x : A} \Pi_{y : B(x)} T}
    \infer2{\Gamma \vdash \existselim \, E_1 \, E_2 : T}
  \end{prooftree}
 \]
provided $x$ and $y$ do not occur free in $T$.

Formulating an elimination rule for the Sigma type that abstracts away the
witness and imposes an abstraction barrier leads us to a ``weaker" Sigma
type. We call these existentials and denote them by $\exists x : A, \, B(x)$
to distinguish them from the usual (strong) Sigma type, $\Sigma_{x : A}, B(x)$.
We also use $(|x, \, y|)$ instead of the $(x, \, y)$ to denote the data
constructor.
...
\item \textbf{Pi and strong Sigma type} \\
  \[
    \begin{prooftree}
      \hypo{x}
      \hypo{A}
 ...
 \]
  ...
  \[
    ...
    \end{prooftree}  
  \]
  This constructs a pair of some strong Sigma type.
 ...
 \begin{prooftree}
   \hypo{E_1}
   \hypo{E_2}
   \infer2{\existselim \, E_1 \, E_2}
 \end{prooftree}  
 \]

 This models the rule of existential elimination, which when modelled type 
 theoretically, roughly says:
 \[
  \begin{prooftree}
    \hypo{\Gamma \vdash E_1 : \exists x : A, \, B(x) }
    \hypo{\Gamma \vdash E_2 : \Pi_{x : A} B(x) \rightarrow T}
    \infer2{\Gamma \vdash \existselim \, E_1 \, E_2 : T} 
  \end{prooftree}
 \]
 provided $x$ does not occur free in $T$.

 Note how this looks similar to the typing judgments for unpacking existential
 types.
...
\item \textbf{Normalizing under pair constructor} \\
  ...
  \item \textbf{Normalizing under existential pair constructor} \\
   \[
      \begin{prooftree}
        \hypo{\Gamma \vdash E_1 \Downarrow \nu_1}
        \hypo{\Gamma \vdash E_2 \Downarrow \nu_2}
  ...
  \item \textbf{Existential eliminator} \\
   \[
      \begin{prooftree}
        \hypo{\Gamma \vdash E_1 \Downarrow \nu_1}
        \hypo{\Gamma \vdash E_2 \Downarrow \nu_2}
        \infer2{\Gamma \vdash \existselim \, E_1 \, E_2 \Downarrow \existselim \, \nu_1, \, \nu_2}
      \end{prooftree}
    \]
...
\[
\begin{prooftree}
  \hypo{\Gamma \vdash E_1 \Rightarrow \exists x : \tau, \, \tau'(x)}
  \hypo{\Gamma \vdash E_2 \Rightarrow \Pi_{x : \tau} \Pi_{x' : \tau'(x)} \tau''}
  \hypo{x, \, x' \notin \FV(\tau'')}
  \infer3{\Gamma \vdash \existselim \, E_1 \, E_2 \Rightarrow \tau''}
\end{prooftree}
\]
Here we write $x, \, x' \notin \FV(\tau'')$ to emphasize that $x$ and $x'$ 
should not occur free in the output type that is $\tau''$.

Note how this corresponds to the rule of existential elimination in natural
deduction as we discussed in the intro
\[
\begin{prooftree}
  \hypo{\Gamma \vdash \exists x, \, \varphi(x)}
  \hypo{\Gamma \vdash \forall x, \, \varphi(x) \rightarrow \psi}
