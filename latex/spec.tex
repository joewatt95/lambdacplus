...
\begin{definition} [Well formed type]
...
\begin{definition} [Bidrectional typechecking]
  ...
  %       \hypo{\Gamma \vdash A \Downarrow \tau}
  %       \hypo{(x, \, \tau, \, \text{undefined}) :: \Gamma \vdash B(x) \Leftarrow \Type}
  %       \infer3{\Gamma \vdash \Sigma_{x : A}B(x) \Rightarrow \Type}
  %     \end{prooftree}
  %   \]
  ...
  %       \infer2{\Gamma \vdash \langle  E_1, \, E_2 \rangle \Leftarrow \Sigma_{x : \tau_1}\tau_2(x)}
  %     \end{prooftree}
  %   \]
...
\begin{align*}
  \begin{prooftree}
    \hypo{\Gamma \vdash E \Leftarrow \tau}
    \hypo{\tau \equiv_\beta \tau'}
    \infer2[\text{Conv}]{\Gamma \vdash E \Leftarrow \tau'}
  \end{prooftree}
\end{align*}

where we use $\tau \equiv_\beta \tau'$ to mean that we may obtain one from the other
