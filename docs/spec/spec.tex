...
\footnote{
  ...
  implemented in the language (yet).
}
...
For this, we augment CoC with the concept of a context (our take on an environment),
...
% provided $x$ does not occur free in $\Gamma$ and $\psi$.
  ...
  of large types like type constructors found in \texttt{Kind}.
  }
...
\subsubsection{Big step transition}
...
As with any other project, $\lambda C+$ has some limitations.
Most of them are a result of dependent type theory being a complicated subject.
Since we are far from experts in this field, we have opted for simplicity to
keep our semantics and implementation manageable.
Consequently, some trade-offs had to be made.

Some of these, like \cref{lack_of_strong_norm} are primarily metatheoretic in
nature and not much of a practical concern for users.
Others like \cref{lack_of_real_type_infer} and \cref{conflaiton_prop_type}
unfortunately do hurt the user-friendliness of our implementation.

% The limitations of the language \cref{lack_of_strong_norm} are theoretical in
% nature and offers some avenues for future work.
% and don't really affect the usability of our language.
% Others like \cref{lack_of_real_type_infer} 

\subsection{Lack of strong normalization and consistency}
\label{lack_of_strong_norm}
...
Thus being able to derive Girard's paradox compromises both strong normalization
...
It should be stressed that this does not have too much of an impact on the
practical usability of the language and is more of a metatheoretic concern.
...
\label{conflaiton_prop_type}

$\lambda C+$ stays true to the Curry-Howard correspondence by happily identifying
propositions with types.
Unfortunately, treating \texttt{Prop} as an abbreviation for \texttt{Type} can 
be unintuitive for users who are familiar with first (or higher order) logic.
...
In other words, there are some types for which we want to think of as sets and
others as propositions.

In logic, there is a clear semantic distinction between terms 
(which denote elements of a set) and well formed formulae (ie logical propositions).
Formulae like $\exists x \in A, \, A$ are considered invalid.

Though the Curry-Howard correspondence is able to give meaning to the type
\verb|exists x : A, A|, this may be counter-intuitive. 
Ideally, we would like to report an error to the user if such types are
formed.

Unfortunately, there is no easy way to address this as this is baked into
the underlying theory of CoC.
Indeed in the original paper that introduced CoC \cite{coc_coquand_huet},
Coquand and Huet explain that
\begin{quote}
  Here, in the spirit of the so-called ``Curry-Howard isomorphism,'' 
  [\texttt{Type}]
  \footnote{Note that they use $*$ instead of \texttt{Type}, as is customary in the literature.}
  must
  be thought of as the type of all types, and the type of all propositions.
\end{quote} 

% Another downside of confliating \texttt{Prop} and \texttt{Type} is that
% our error messages can be rather cryptic.
% For instance, when a user gives a wrong proof for a theorem, like
% \begin{verbatim}
%   theorem h : forall (A : Prop) (B : Prop), A /\ B -> A :=
%     fun A B a_and_b => snd a
% \end{verbatim}

% we simply
% complain that there's a type mismatch in that the expression they provided has
% a different type from the one they claimed it was supposed to have.
% This exposes the underlying Pi and Sigma types to the user, which can be
% overwhelming.
% While this is perfectly acceptable according to Curry-Howard, the usability of
% our language suffers as a result.
% What we would ideally prefer is to o

% Since we lose information about whether a type variable \texttt{A} was introduced
% as a \texttt{Prop} or \texttt{Type}, our error messages

One way to fix this is to further stratify the
...
\texttt{Kind}.
The typing rules would then need to be adjusted to capture the semantic difference
between \texttt{Type} (which behaves like a universe of sets) and \texttt{Prop}
(which behaves like a universe of logical propositions).
The Calculus of  Inductive Constructions (CiC) \cite{intro_to_cic} which lies
at the heart of proof assistants like Coq and Lean adopts an approach like this.

To avoid complicating our semantics and implementation, we chose not to
implement this.
However, we would like to revisit this in the future.
